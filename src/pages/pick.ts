...
function renderResults(container: HTMLElement, results: PickResult[], elapsed: number) {
  ...
  const sections: Array<[string, string, PickResult[]]> = [
    ['best', '大吉之日', best.slice(0, 20)],
    ['good', '吉日', good.slice(0, 20)],
  ];
  for (const [cls, title, rows] of sections) {
    if (rows.length === 0) continue;
    const section = createElement('div', 'result-section');
    section.innerHTML = `<h3 class="section-title ${cls}">${title}</h3>`;
    const grid = createElement('div', 'result-grid');
    rows.forEach(r => grid.appendChild(createResultCard(r)));
